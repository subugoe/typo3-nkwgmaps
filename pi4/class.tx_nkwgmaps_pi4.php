...
class tx_nkwgmaps_pi4 extends tx_nkwgmaps {
	public $prefixId = 'tx_nkwgmaps_pi4';
	public $scriptRelPath = 'pi4/class.tx_nkwgmaps_pi4.php';
	...
	public function main($content, $conf) {
		...
		$conf['ff']['addressbooksource']['uid'] = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'source', 'addressdata');
			// get data from DB
		$cntMarker = 0;
		$addressbooksourceUids = explode(',', $conf['ff']['addressbooksource']['uid']);
		foreach ($addressbooksourceUids as $uid) {
		...
		if (!$fail) {
			...
			$js = tx_nkwgmaps::multiGmapsJS($conf);
		} else {
			$tmp = '<p>' . $msg . '</p>';
		}
